...
Page({
  ...
  loadOrders: function() {
    ...
    API.getUserOrders().then(function(orders) {
      var mapped = orders.map(function(o) {
        ...
        // Demo logistics for completed orders
        var log = null
        if (o.status === 'Completed' && o.orderId === 'ORD003') {
          log = {
            carrier: '顺丰速运', icon: '📦', trackingNum: 'SF1234567890',
            statusBadge: 'transit', statusLabel: '运输中',
            estimated: '预计明日送达',
            steps: [
              { label: '已下单', active: true },
              { label: '已发货', active: true, current: true },
      ...
      success: function(res) {
        if (res.confirm) {
          ...
          for (var i = 0; i < orders.length; i++) {
            if (orders[i].orderId === e.currentTarget.dataset.id) {
              orders[i].status = 'Completed'
              orders[i].statusClass = 'status-completed'
              orders[i].statusLabel = '已完成'
              break
            }
          }
  ...
  goRoomControl: function(e) {
    wx.navigateTo({ url: '/pages/room-control/room-control' })
  },
      ...
      if (orders[i].orderId === id && orders[i].logistics) {
        this.setData({
          ...
          logisticsData: {
            ...
            timeline: [
              { title: '已揽收', time: '2026-06-06 14:00', status: 'done', active: true },
              { title: '已到达广州分拣中心', time: '2026-06-06 18:30', status: 'done', active: true },
              { title: '派送中', time: '2026-06-07 08:00', status: 'current', active: true },
              { title: '已签收', time: '预计今天', status: 'pending', active: false }
            ]
          }
        })
        break
