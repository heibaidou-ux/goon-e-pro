      ...
      <t-row :gutter="16" style="margin-bottom:20px">
        <t-col :span="6">
          <t-card title="场景列表" :bordered="true">
            ...
            <div v-for="scene in sceneList" :key="scene.sceneId" class="scene-item" @click="selectScene(scene)">
...
function normalizeScene(s: any) {
  return {
    ...
    name: s.name || '',
    label: s.label || '',
    triggerType: s.triggerType || s.trigger_type || '',
    applicableRoomTypes: s.applicableRoomTypes || s.applicable_room_types || [],
...
async function loadData() {
  ...
  loadError.value = ''
  try {
    const scenes = await iotApi.scenes().catch(() => [] as any[])
    sceneList.value = (scenes || []).map(normalizeScene)
  } catch (e: any) {
    loadError.value = '加载场景数据失败: ' + (e.message || e)
  } finally {
    loading.value = false
  }
}
