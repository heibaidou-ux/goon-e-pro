    ...
    <template v-else>
      ...
      <t-card :bordered="true">
        ...
        <t-table :data="filteredAssets" :columns="columns" row-key="assetId" hover stripe>
          <template #originalValue="{ row }">¥{{ (row.originalValue || 0).toLocaleString() }}</template>
          <template #currentValue="{ row }">¥{{ (row.currentValue || 0).toLocaleString() }}</template>
          <template #status="{ row }">
        ...
        <t-empty v-if="filteredAssets.length === 0" description="暂无固定资产数据" style="padding:40px" />
      </t-card>
    </template>
    ...
    <t-drawer v-model:visible="detailVisible" :header="`${selectedAsset?.name} 详情`" size="400px" :footer="false">
      <div v-if="selectedAsset" class="detail-sections">
        <div class="detail-section">
          ...
          <div class="detail-row"><span>原值</span><span class="price">¥{{ (selectedAsset.originalValue || 0).toLocaleString() }}</span></div>
          <div class="detail-row"><span>当前净值</span><span class="price">¥{{ (selectedAsset.currentValue || 0).toLocaleString() }}</span></div>
        ...
        <div class="detail-section">
          ...
          <div class="detail-row"><span>残值率</span><span>{{ selectedAsset.residualRate != null ? (selectedAsset.residualRate * 100).toFixed(0) : '—' }}%</span></div>
...
const monthlyDepreciation = computed(() => {
  ...
  if (!a.depreciationYears || a.depreciationYears <= 0) return 0
  return Math.round(a.originalValue * (1 - (a.residualRate || 0)) / a.depreciationYears / 12)
})

const assetStats = computed(() => {
  const list = assets.value
  const total = list.reduce((s: number, a: any) => s + (a.originalValue || 0), 0)
  return [
    { label: '资产总数', value: list.length, color: '#0052D9' },
    { label: '资产原值', value: '¥' + total.toLocaleString(), color: '#E37318' },
    { label: '使用中', value: list.filter(a => a.status === '使用中').length, color: '#00A870' },
    { label: '月折旧合计', value: '¥' + list.reduce((s: number, a: any) => {
      if (!a.depreciationYears || a.depreciationYears <= 0) return s
      return s + Math.round(a.originalValue * (1 - (a.residualRate || 0)) / a.depreciationYears / 12)
    }, 0).toLocaleString(), color: '#D54941' },
  ]
})
...
async function confirmDispose() {
  ...
  try {
    await financeApi.updateAsset(disposeAsset.value.assetId, { status: '已报废' })
    if (disposeAsset.value) disposeAsset.value.status = '已报废'
  } catch (e: any) {
    console.error('处置资产失败:', e)
  }
...
async function loadAssets() {
  ...
  loadError.value = ''
  try {
    const result = await financeApi.listAssets({ page_size: 100 })
    assets.value = result.items || []
  } catch (e: any) {
    loadError.value = '加载固定资产数据失败: ' + (e.message || e)
  } finally {
    loading.value = false
  }
}
