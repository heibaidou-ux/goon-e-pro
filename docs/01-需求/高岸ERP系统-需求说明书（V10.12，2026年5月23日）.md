...
- 2026年5月5日盈隆店项目组评审意见
- 2026年5月7日评审意见
- 2026年5月23日客人端交互原型评审纪要（王总、珊珊、小东、于总、维姐）
...
**客人端小程序**是面向最终消费者的核心触点，覆盖从引流到结算的全链路自助操作。所有功能均通过小程序内的"首页-门店-我的"三个底部Tab组织：首页展示推荐包间和营销活动，门店页承载预约和点单流程，我的页集中管理订单、会员、优惠券等信息。验券页面支持手动输入券码和二维码扫码两种方式，同时展示平台券（美团/抖音）和系统发放券，方便客人核销。
...
**功能描述**：系统为不同角色提供定制化工作台，展示该角色最关注的数据和功能入口，实现角色化信息聚合。

**投资人工作台**（对应 3.1.4 投资者关系页面）：
- 展示个人投资的门店列表、持股比例
- 门店财务概览（本月营收、本月利润、累计分红）
- 近6个月利润趋势图
- 投资门店排名对比
- 重大告警通知（设备离线、大额对账差异等）
...
- 全品牌总览（总营收、总支出、总利润）
- 各店经营数据对比看板
- 关键KPI仪表盘（营收达标率、成本率、客流量）
- 待审批事项汇总（需总经理审批的支出/活动等）
- AI预警摘要（营收异常、成本异常等）

**财务工作台**：
- 今日待处理事项汇总（待对账工单数、待审批支出数、待处理差异）
- 月结进度状态（本月月结是否完成、上月月结确认状态）
- 近7日营收趋势图
- 各平台对账差异统计
- 快捷操作入口：对账中心、月结操作、支出审批、报表查看
...
**业务规则**：
- 各角色工作台仅展示该角色权限范围内的数据
- 工作台为角色登录后的默认首页
- 工作台数据实时或准实时更新（不超过30秒延迟）
- 工作台布局支持角色预设，不支持个人定制
...
消费进行中，客人可通过小程序端对房间设备进行自助控制：

- **智能门锁**：已到店客人可通过小程序一键开门（网关在线时），无需使用密码
- **空调控制**：调节目标温度（16-30℃）、开关机、切换模式（制冷/制热/送风）
- **灯光控制**：切换场景模式（品茶暖色/会议冷白/K歌彩灯），支持开关和亮度调节
- **背景音乐**：播放/切换/暂停曲目，调节音量
- **窗帘控制**：打开/关闭/停止

**业务规则**：
- 客人控制权限仅限本人当前使用的包间
- 客人操作优先级高于场景联动，手动操作后场景联动暂停15分钟
- 客人可随时通过小程序切换回自动场景模式
- 控制指令通过IoT平台下发，离线时提示客人使用物理开关（详见3.8.1）
...
- 外卖订单支持**物流追踪**：订单详情页展示物流进度（待发货/运输中/已送达），显示快递公司、运单号、预计送达时间
- 物流信息可由店员在订单管理中录入（选择快递公司、填写运单号），系统自动更新物流状态
- 客人端小程序"我的订单"页面可查看物流进度条和快递单号，点击运单号可复制
...
**门店巡检与整改泳道图**展示巡店计划生成、执行、异常处理和整改的完整流程。

---

#### 3.3.6 店员工作台

**功能描述**：店员登录后默认展示今日工作台，集中呈现门店运营概览、快捷操作入口和待办事项，实现常用功能一键可达。
...
**房态总览卡片**：
- 展示各房态数量统计（空闲/使用中/待打扫/维修/已预定），使用颜色区分（绿/蓝/黄/红/紫）
- 点击跳转至房态管理页面查看详情

**今日概况统计**：
- 今日订单数、进行中订单、待开始订单、已结束订单
- 今日营收、今日应收

**快捷操作入口**（两行三列网格）：
- 包间预订：为客人代客预订包间（详见 3.3.3 房态管理）
- 开门控制：远程打开指定包间门锁
- 保洁任务：查看待处理保洁任务
- 商品管理：管理门店商品上下架与库存
- 对账中心：处理对账差异工单
...
**待办事项**：
- 按类型分组展示待办任务（对账差异、审批请求、保洁确认、巡检整改等）
- 显示待办数量，点击跳转至对应处理页面
- 任务按优先级排序，超时任务高亮标注

**业务规则**：
- 工作台数据实时更新，刷新间隔不超过30秒
- 员工仅看到自己门店的数据
- 店长端在店员端基础上增加审批入口和门店数据看板
- 底部导航栏支持快速切换：首页、房态、订单、我的

**关联页面**：待办任务中心（3.3.7）、房态管理（3.3.3）、保洁任务管理（3.3.4）

---

#### 3.3.7 待办任务中心

**功能描述**：集中展示当前门店所有待处理任务，按类型分组，支持店员逐一处理和任务跳转。

**任务类型**：

| 任务类型 | 来源 | 处理方式 |
|---------|------|---------|
| 对账差异工单 | 平台自动对账生成 | 核实差异，提交处理说明 |
| 保洁任务 | 退房后自动生成 | 接单→到达→清洁→完成 |
| 审批请求 | 请假/排班/支出申请 | 通过/驳回 |
| 巡店任务 | 系统按计划生成 | 逐项检查并提交报告 |
| 整改任务 | 巡店异常创建 | 整改完成提交确认 |

**操作流程**：
1. 店员进入待办任务中心，查看分组任务列表
2. 每种任务类型显示待处理数量，点击进入对应处理页面
3. 任务按时间排序，超时任务置顶高亮
4. 处理完成后任务自动从待办列表移除
...
**功能描述**：管理后台对全部类型订单进行统一管理和跟踪，支持空间租用订单、商品零售订单、茶品订单等多类型订单的查看、状态更新和物流管理。

**订单类型**：

| 订单类型 | 来源 | 说明 |
|---------|------|------|
| 空间租用订单 | 客人预约包间 | 含房间信息、时段、费用、支付状态 |
| 商品零售订单 | 茶吧/小程序购买商品 | 含商品明细、物流信息、自提/外卖方式 |
| 茶品订单 | 到店茶饮消费 | 含茶品明细、包间号、制作状态 |

**操作流程**：
1. 管理后台默认展示全部订单列表，按时间倒序排列
2. 支持按订单类型（空间租用/商品/茶品）、订单状态、时间范围筛选
3. 点击订单查看详情（订单信息、支付记录、物流信息、操作日志）
4. 商品订单支持物流信息录入：选择快递公司、填写运单号、更新发货状态
5. 订单状态异常时标记并推送至店员处理

**业务规则**：
- 空间租用订单与房态管理联动，订单状态变更自动更新房态
- 商品订单支持物流进度跟踪（详见 3.3.2 商品零售）
- 订单数据实时同步至财务域
...
- 系统发放优惠券在客人端小程序"我的优惠券"页面集中展示
- 验券页面支持平台券和系统券统一核销，同时提供二维码扫码输入
...
- 首次充值 → 发放10张¥50现金券（有效期180天）→ 核销后标记"首充转化"
...
店员端提供门店应收款视图，方便门店跟踪未到账款项：

- **应收款列表**：展示所有未到账收入记录，包括订单编号、来源渠道、应收金额、实收金额、差异、状态
- **状态维度**：待确认 / 部分到账 / 已到账 / 差异待处理
- **操作功能**：店员可查看应收款明细，对差异项提交对账工单
- **渠道筛选**：按收入渠道（包间预订/零售/团购套餐/会员充值等）筛选应收款

**业务规则**：
- 应收款数据来源于系统收入流水与各平台结算单的比对结果
- 订单完成后未确认到账的收入自动计入应收款
- 已到账款项自动从应收款列表移除，转入收入流水
- 长期未到账（超过30天）的应收款自动告警推送至财务
...
管理后台提供在线数据分析看板，支持多维度交互式分析：

- **趋势分析**：按日/周/月展示营收、订单量、客流量趋势，支持时间范围选择
- **对比分析**：门店间经营数据横向对比（收入/支出/利润/客单价）
- **构成分析**：收入构成（空间租用/零售/会员充值等）、支出构成（采购/人力/水电等）
- **目标达成**：实际经营数据 vs 目标值对比，偏差自动标红
- **数据导出**：分析图表及原始数据支持导出为Excel
...
店员可通过移动端查看门店所有IoT设备的实时状态：

- **设备列表**：按房间分组展示所有设备，显示设备名称、类型（门锁/空调/灯光/窗帘/音乐）、在线状态
- **状态指示**：在线（绿色）/ 离线（红色）/ 故障（橙色），列表顶部汇总各状态数量
- **快捷操作**：在线设备支持一键开门、开关灯等常用操作
- **故障告警**：离线或故障设备在列表中置顶高亮显示，点击查看故障详情和建议操作
- **批量操作**：支持全开/全关灯光、空调等批量控制（仅限空闲房间）

**业务规则**：
- 监控面板数据实时刷新（默认每10秒）
- 店员仅可查看和操作本门店设备
- 使用中的房间设备仅限查看状态，不可远程操作（避免干扰客人）
...
| 场景 | 触发条件 | 设备动作 | 适用 |
|------|---------|---------|------|
| 欢迎场景 | 客人开门 | 开灯→开空调（预设温度）→播放轻音乐→语音播报"欢迎光临高岸空间" | 所有包间 |
| 品茶场景 | 包间使用中 | 灯光调至暖色（3000K）→ 保持空调 | 茶室 |
| 会议场景 | 手动切换 | 灯光调至冷白（4000K）→ 静音 | 会议室 |
| K歌场景 | 手动切换 | 灯光调至彩灯模式 → 音响开启 | 娱乐包间 |
| 结束前提醒 | 订单结束前3分钟 | 语音播报"本次消费即将结束" | 所有包间 |
| 节能场景 | 退房 | 关灯→关空调→关音乐→关窗帘→语音播报"已结束，欢迎下次光临" | 所有包间 |
...
| 设备类型 | 功能需求 | 离线容错需求 |
|---------|---------|-------------|
| **智能门锁** | 接收ERP下发的临时密码并验证开锁；支持小程序推送密码至客人；客人可通过小程序一键开门（网关在线时）；开门事件上报至系统 | 支持离线动态密码机制：即使网关断网，门锁也能根据时间同步算法验证密码有效性；离线时小程序开门不可用，自动提示使用密码开门 |
...
| **智能音箱/语音播报** | 场景联动语音提示：进入房间时播报"欢迎光临高岸空间"；订单结束前3分钟播报"本次消费即将结束"；订单结束后播报"已结束，欢迎下次光临"。语音内容可通过管理后台在线编辑更换 | 本地预存语音文件，网关断网时仍可触发基础播报 |
...
| 版本 | 日期 | 修订内容 | 修订人 |
|------|------|---------|--------|
| V10.12 | 2026-05-23 | 根据原型评审结果更新：3.1.5新增各角色专属工作台（投资人/总经理/财务/技师）；3.3.1新增客人自助房间控制（门锁/空调/灯光/音乐/窗帘）；3.3.2新增外卖订单物流追踪；3.3.6新增店员工作台；3.3.7新增待办任务中心；3.3.8新增订单管理中心（空间租用/商品/茶品多类型统一管理）；3.5.4新增应收款管理（店员端门店应收视图）；3.6.6新增在线数据分析看板；3.8.1新增店员端设备监控面板；客人端主页/订单页/验券页根据5月23日交互原型评审意见完成修改（贯穿式设计、余额合并至图标、订单卡片增加设备控制入口、优惠券增加高德地图平台） | Claude Code |
