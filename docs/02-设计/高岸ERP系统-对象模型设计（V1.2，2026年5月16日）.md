...
**文档状态**：草稿
**编制依据**：
- 《高岸ERP系统-需求说明书（V10.11，2026年5月16日）》全文，特别是第二章业务域划分、第三章功能需求
...
| source | Enum | 是 | MT(美团)/DY(抖音)/DP(大众点评)/System(系统赠送) | — |
...
- 退单时优惠券按规则退回或作废
- 核销数据自动回流客户标签
- 系统发放券（source=System）由充值赠送等活动自动生成，不经过CouponTemplate
- 系统发放券面值固定为整数（如¥50），全场通用，不限制适用商品范围
...
| **CouponSource** | MT(美团) / DY(抖音) / DP(大众点评) / System(系统赠送) | Coupon |
...
| 版本 | 日期 | 修订内容 | 修订人 |
|------|------|---------|--------|
| V1.2 | 2026-05-16 | 7.2.3 Coupon实体新增source属性（区分平台券和系统赠送券），新增CouponSource枚举，补充系统发放券业务约束（固定面值、全场通用、不依赖CouponTemplate） | Claude Code |
